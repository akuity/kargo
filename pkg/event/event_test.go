...
func TestSource(t *testing.T) {
	testCases := map[string]struct {
		...
		// Currently don't need any others but leaving as a test table just in case
	}
...
func TestToCloudEvent(t *testing.T) {
	// NOTE(thomastaylor312): This is mostly meant as a smoke test to make sure something isn't
	// seriously broken with the event conversion logic. If we have any other issues, we can test
	// every event
	testCases := map[string]struct {
		eventMeta Meta
		// Because this is generic, we need to provide an empty type to deserialize back into
		eventParsed    Meta
		expectedType   string
		expectedSource string
	}{
		"promotion event": {
			eventMeta: &PromotionCreated{
				Common: Common{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// Verify data is set
			err := cloudEvent.DataAs(&tc.eventParsed)
			require.NoError(t, err)
			require.Equal(t, tc.eventMeta, tc.eventParsed, "Should get the same data after round trip")
		})
	}
}
