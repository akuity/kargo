...
func TestFreightVerification_GetTriggeredByPromotion(t *testing.T) {
	testCases := map[string]struct {
		...
		"with promotion": {
			verification: FreightVerification{
				AnalysisTriggeredByPromotion: stringPtr("test-promotion"),
			},
			expected: stringPtr("test-promotion"),
		},
		"without promotion": {
			verification: FreightVerification{},
			expected:     nil,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			result := tc.verification.GetTriggeredByPromotion()
			require.Equal(t, tc.expected, result)
		})
	}
}

func TestFreightVerification_SetTriggeredByPromotion(t *testing.T) {
	verification := &FreightVerification{}

	// Test setting a value
	verification.SetTriggeredByPromotion(stringPtr("test-promotion"))
	require.Equal(t, stringPtr("test-promotion"), verification.AnalysisTriggeredByPromotion)

	// Test setting nil
	verification.SetTriggeredByPromotion(nil)
	require.Nil(t, verification.AnalysisTriggeredByPromotion)
}
	...
	testCases := map[string]struct {
		...
		"complete verification": {
			verification: FreightVerification{
				...
				FinishTime:                   timePtr(time.Date(2024, 1, 1, 11, 0, 0, 0, time.UTC)),
				AnalysisRunName:              stringPtr("test-analysis"),
				AnalysisTriggeredByPromotion: stringPtr("test-promotion"),
			},
			expected: map[string]string{
