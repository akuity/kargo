...
// TestGetClient exercises the client end to end against a fake API server:
// transport wiring (bearer token and CLI version headers, URL construction)
// and, critically, wire fidelity of the display path that motivated the
// migration -- a Stage fetched through the client and round-tripped into
// kargoapi.Stage must reproduce the server's response exactly, with no
// phantom empty objects for unset optional fields.
func TestGetClient(t *testing.T) {
	...
	apiClient, err := GetClient(srv.URL, "test-token", false)
	require.NoError(t, err)
