...
func (w *webhook) Default(
	...
	if promotionRequest.Spec.Origin != nil && promotionRequest.Spec.Freight == "" {
		...
		if freight, err = w.resolveOriginToFreight(ctx, promotionRequest); err != nil {
			return err
		}
	...
	// one generated by Kargo, exactly as the Promotion webhook does. This is
	// also what lets a caller promote by origin: the name embeds a short hash
	// of the Freight, which such a caller could not have computed.
...
// that the origin's auto-promotion selection policy would choose for the
// request's Stage. Returns an error (denying admission) if the Stage does not
// exist or no Freight is available. The selection itself is shared with the
// Promotion defaulting webhook, so promote-by-origin picks the same Freight
// regardless of which resource carries the origin.
