...
func TestNewKubernetesRolesDatabaseWiresAuthorizer(t *testing.T) {
	...
	db, ok := NewKubernetesRolesDatabase(plain, plain, RolesDatabaseConfig{}).(*rolesDatabase)
	require.True(t, ok)
	...
	db, ok = NewKubernetesRolesDatabase(authorizing, plain, RolesDatabaseConfig{}).(*rolesDatabase)
	require.True(t, ok)
...
func TestVerifyBindingNotEscalating(t *testing.T) {
	...
	t.Run("nil authorizer disables the check", func(t *testing.T) {
		require.NoError(t, verifyBindingNotEscalating(t.Context(), nil, testProject, resources))
	})

	t.Run("no bound roles means nothing to escalate", func(t *testing.T) {
		require.NoError(t, verifyBindingNotEscalating(t.Context(), &fakeAuthorizer{}, testProject, Resources{}))
	})
	...
	t.Run("forbidden when requester lacks a permission in the bound role", func(t *testing.T) {
		...
		err := verifyBindingNotEscalating(t.Context(), authz, testProject, resources)
		require.Error(t, err)
		require.True(t, apierrors.IsForbidden(err))
		require.ErrorContains(t, err, `resource "secrets"`)
	})
...
func TestGrantRoleToUsersBlocksEscalation(t *testing.T) {
	...
	newDB := func(authz *fakeAuthorizer) (RolesDatabase, client.Client) {
		...
		return NewKubernetesRolesDatabase(
			&authorizingFakeClient{Client: c, authz: authz}, c, RolesDatabaseConfig{},
		), c
	}
