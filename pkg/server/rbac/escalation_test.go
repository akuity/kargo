...
func TestNewKubernetesRolesDatabaseWiresAuthorizer(t *testing.T) {
	...
	db, ok := NewKubernetesRolesDatabase(plain).(*rolesDatabase)
	require.True(t, ok)
	...
	db, ok = NewKubernetesRolesDatabase(authorizing).(*rolesDatabase)
	require.True(t, ok)
...
func TestGrantRoleToUsersBlocksEscalation(t *testing.T) {
	...
	newDB := func(authz *fakeAuthorizer) (RolesDatabase, client.Client) {
		...
		return NewKubernetesRolesDatabase(
			&authorizingFakeClient{Client: c, authz: authz},
		), c
	}
...
// TestCreateBlocksEscalation is the end-to-end guard: a user who cannot read
// secrets must not be able to create a Role whose rules grant secrets access.
func TestCreateBlocksEscalation(t *testing.T) {
	newKargoRole := func() *rbacapi.Role {
		return &rbacapi.Role{
			ObjectMeta: metav1.ObjectMeta{Namespace: testProject, Name: testKargoRoleName},
			Rules: []rbacv1.PolicyRule{{
				APIGroups: []string{""},
				Resources: []string{"secrets"},
	...
	t.Run("blocked when requester lacks secrets", func(t *testing.T) {
		...
		db := NewKubernetesRolesDatabase(&authorizingFakeClient{Client: c, authz: &fakeAuthorizer{
			allow: func(_ string, gvr schema.GroupVersionResource, _ string) bool {
				return gvr.Resource != "secrets"
			},
		}})
		_, err := db.Create(t.Context(), newKargoRole())
		require.Error(t, err)
		require.True(t, apierrors.IsForbidden(err))

		// Nothing should have been created as a side effect.
		require.True(t, apierrors.IsNotFound(c.Get(t.Context(), objKey, &corev1.ServiceAccount{})))
		require.True(t, apierrors.IsNotFound(c.Get(t.Context(), objKey, &rbacv1.Role{})))
	})

	t.Run("allowed when requester holds the permissions", func(t *testing.T) {
		c := fake.NewClientBuilder().WithScheme(scheme).Build()
		db := NewKubernetesRolesDatabase(&authorizingFakeClient{Client: c, authz: &fakeAuthorizer{}})
		_, err := db.Create(t.Context(), newKargoRole())
		require.NoError(t, err)
		require.NoError(t, c.Get(t.Context(), objKey, &rbacv1.Role{}))
	})
}

// TestUpdateBlocksEscalation is the end-to-end guard: a user who cannot read
// secrets must not be able to rewrite a Role's rules to grant secrets access.
func TestUpdateBlocksEscalation(t *testing.T) {
	newKargoRole := func() *rbacapi.Role {
		return &rbacapi.Role{
			ObjectMeta: metav1.ObjectMeta{Namespace: testProject, Name: testKargoRoleName},
			// A claim binding an identity, submitted alongside the escalating rule.
			Claims: []rbacapi.Claim{{Name: "email", Values: []string{"attacker@evil.example"}}},
			Rules: []rbacv1.PolicyRule{{
				APIGroups: []string{""},
				Resources: []string{"secrets"},
	...
	t.Run("blocked when requester lacks secrets", func(t *testing.T) {
		...
		_, err := db.Update(t.Context(), newKargoRole())
		require.Error(t, err)
		require.True(t, apierrors.IsForbidden(err))

		// The stored Role must not have gained the secrets rule.
		role := &rbacv1.Role{}
		require.NoError(t, c.Get(t.Context(), objKey, role))
		require.Empty(t, role.Rules)

		// The check runs before anything is persisted, so the binding (claims)
		// must not have been written to the ServiceAccount either.
		sa := &corev1.ServiceAccount{}
		require.NoError(t, c.Get(t.Context(), objKey, sa))
		got, err := rbacapi.OIDCClaimsFromAnnotationValues(sa.Annotations)
		require.NoError(t, err)
