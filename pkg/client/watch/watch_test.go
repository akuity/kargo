...
func TestPromoteToStage(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "successful promotion with freight",
			freight:      "freight-123",
			freightAlias: "",
			serverResponse: func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, http.MethodPost, r.Method)
				assert.Equal(t, "/v2/projects/test-project/stages/test-stage/promotions", r.URL.Path)
				...
				assert.Equal(t, "freight-123", req.Freight)
				assert.Empty(t, req.FreightAlias)

				w.WriteHeader(http.StatusOK)
				_ = json.NewEncoder(w).Encode(PromoteToStageResponse{
					Promotion: &kargoapi.Promotion{
						ObjectMeta: metav1.ObjectMeta{Name: "promo-123"},
					},
				})
			},
		},
		{
			name:         "successful promotion with freight alias",
			freight:      "",
			freightAlias: "my-alias",
			serverResponse: func(w http.ResponseWriter, r *http.Request) {
				body, _ := io.ReadAll(r.Body)
				var req PromoteToStageRequest
				_ = json.Unmarshal(body, &req)
				assert.Empty(t, req.Freight)
				assert.Equal(t, "my-alias", req.FreightAlias)

				w.WriteHeader(http.StatusCreated)
				_ = json.NewEncoder(w).Encode(PromoteToStageResponse{
					Promotion: &kargoapi.Promotion{
						ObjectMeta: metav1.ObjectMeta{Name: "promo-456"},
					},
				})
			},
		},
		...
		},
		{
			...
			serverResponse: func(w http.ResponseWriter, _ *http.Request) {
				...
				_, _ = w.Write([]byte(`{"error": "stage not found"}`))
			},
			expectError:   true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			promotion, err := client.PromoteToStage(ctx, "test-project", "test-stage", tt.freight, tt.freightAlias)
...
func TestPromoteToStage_NoToken(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	promotion, err := client.PromoteToStage(ctx, "test-project", "test-stage", "freight-123", "")
	require.NoError(t, err)
	require.NotNil(t, promotion)
}
...
func TestPromoteDownstream(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			serverResponse: func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, http.MethodPost, r.Method)
				assert.Equal(t, "/v2/projects/test-project/stages/test-stage/promotions/downstream", r.URL.Path)

				w.WriteHeader(http.StatusOK)
				_ = json.NewEncoder(w).Encode(PromoteDownstreamResponse{
					Promotions: []*kargoapi.Promotion{
						{ObjectMeta: metav1.ObjectMeta{Name: "promo-1"}},
						{ObjectMeta: metav1.ObjectMeta{Name: "promo-2"}},
					},
				})
			},
			expectCount: 2,
		...
		{
			name: "multi-status with partial success",
			serverResponse: func(w http.ResponseWriter, _ *http.Request) {
				...
				_ = json.NewEncoder(w).Encode(PromoteDownstreamResponse{
					...
					Errors: "failed to promote to stage-2",
				})
			},
			expectError:   true,
			errorContains: "partial failure",
			expectCount:   1,
		},
		{
			name: "server error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			promotions, err := client.PromoteDownstream(ctx, "test-project", "test-stage", "freight-123", "")

			if tt.expectError {
				require.Error(t, err)
				assert.Contains(t, err.Error(), tt.errorContains)
				if tt.expectCount > 0 {
					assert.Len(t, promotions, tt.expectCount)
				}
				return
			}
...
func TestAbortPromotion(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			serverResponse: func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, http.MethodPost, r.Method)
				assert.Equal(t, "/v2/projects/test-project/promotions/test-promotion/abort", r.URL.Path)
				assert.Equal(t, "Bearer test-token", r.Header.Get("Authorization"))
		...
		},
		{
			...
			serverResponse: func(w http.ResponseWriter, _ *http.Request) {
				w.WriteHeader(http.StatusNotFound)
				_, _ = w.Write([]byte(`{"error": "promotion not found"}`))
			},
			expectError:   true,
			errorContains: "unexpected status 404",
		},
		{
			...
			serverResponse: func(w http.ResponseWriter, _ *http.Request) {
				...
				_, _ = w.Write([]byte(`{"error": "promotion already completed"}`))
			},
			expectError:   true,
			errorContains: "unexpected status 409",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := client.AbortPromotion(ctx, "test-project", "test-promotion")
...
func TestAbortPromotion_NoToken(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Empty(t, r.Header.Get("Authorization"))
		w.WriteHeader(http.StatusOK)
	}))
