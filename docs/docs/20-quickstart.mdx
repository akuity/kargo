...
* The `kargo` CLI: To install it, follow these steps.
    ...
    1. Download the appropriate release package for your operating system (e.g., `kargo-linux-amd64` for Linux).
    1. Move the binary to a directory of your choice (e.g. for Linux, consider `/usr/local/bin/`).
    1. Add the directory containing the `kargo` executable to your system's `PATH` environment variable.
