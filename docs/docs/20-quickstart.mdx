...
Argo CD dashboard and three different instances of our demo app on different
`localhost` ports:
...
because the environment-specific branches of your GitOps repo do not exist yet!_
That's okay, though. K8sTA will create those branches for us as we continue this
guide.
...
There are many different ways K8sTA can detect changes that should be progressed
through the environments enumerated by a `Track`. For convenience we will use
the method that requires no additional setup.
...
CD `Application`s representing those environments will become ✅ synced and 💚
healthy.
...
updating the image to be `argoproj/rollouts-demo:green`.
