...
<ReleaseNotesSchema version="v1.5.0" datePublished="2025-05-15" />
...
- Improved Workload Identity Federation support in GKE:
  ...
  Those running Kargo within Elastic Kubernetes Service (EKS) or Google Kubernetes Engine (GKE) may be familiar with Kargo's ability to use [ambient credentials](../40-operator-guide/40-security/50-ambient-credentials.md) when accessing Elastic Container Registry (ECR) and Google Artifact Registry (GAR) repositories, respectively.
