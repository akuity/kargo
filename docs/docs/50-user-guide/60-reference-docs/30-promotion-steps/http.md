...
| `proxy` | `string` | N | An optional URL: the proxy server to send the request through. This overrides Go's [default proxy behavior](https://pkg.go.dev/net/http#ProxyFromEnvironment). __Note:__ The format is more restrictive than the `http_proxy` or `https_proxy` environment variables, because the URL must include a scheme. |
