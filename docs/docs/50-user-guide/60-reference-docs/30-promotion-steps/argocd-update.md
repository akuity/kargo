...
| `apps[].prune` | `boolean` | N | Indicates whether the sync operation initiated for the Argo CD `Application` resource(s) should prune resources that are no longer defined in the `Application`'s sources. Note that Argo CD's `Prune` flag is not a sync option, so it cannot be inherited from an `Application`'s `syncPolicy.syncOptions` the way other sync options are. Defaults to `false`. |
