...
representation of that `Freight`, making it effectively a stable fingerprint.
While this ensures immutability and uniqueness, these names are not intended to
be human-friendly. Additionally, `Freight` aliases are initially assigned random
values.

To make a `Freight` easier to reason about across Stages and environments, users
may want to assign a human-readable, organization-specific alias. This can
improve clarity in promotion pipelines, dashboards, and operational workflows
without sacrificing the safety guarantees provided by immutable identifiers.
