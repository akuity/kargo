...
| Name            | Type     | Required | Description                                                                                                                     |
| --------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `tableName`     | `string` | Y        | Table name (e.g., `incident`, `problem`, `change_request`). |
| `ticketId`      | `string` | Y        | Record `sys_id`.                                  |
| `template`      | `object` | N        | Use Change Management API for Change Requests; otherwise omit. |
| `template.type` | `string` | Y/N      | Required if `template` is specified (`standard`, `emergency`, `normal`). |
