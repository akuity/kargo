...
| `out` | `string` | N | Path where the generated plan file will be saved, relative to the `dir` directory. When specified, the plan can be applied using the `tf-apply` step. |
...
This example saves the plan to a file that can be used with the `tf-apply` step.
This ensures that exactly the changes shown in  the plan are applied, with no
possibility of drift between planning and applying.
