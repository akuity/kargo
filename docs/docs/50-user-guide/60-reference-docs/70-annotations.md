...
| `rbac.kargo.akuity.io/claims` | `ServiceAccount` | Any valid OIDC claim values (e.g., `sub`, `email`, or `groups`) | Maps multiple OIDC claims to a `ServiceAccount`, enabling user-to-ServiceAccount mappings. For more details, refer to the access control sections of the [Operator Guide](../../40-operator-guide/40-security/30-access-controls.md) and [User Guide](../50-security/20-access-controls/index.md).
