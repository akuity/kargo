...
| targetSelector | k8s.io.apimachinery.pkg.apis.meta.v1.LabelSelector |  TargetSelector selects the Target(s) that Freight promoted to this Stage is destined for, matching Targets in the Stage's namespace by label.  When nil, the Stage promotes to a single default destination derived from the Stage itself; no Target resource is required. When set, the Stage promotes to every Target in its namespace whose labels satisfy the selector.  +optional |
