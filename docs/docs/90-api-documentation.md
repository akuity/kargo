...
| discoveryTimestamp | k8s.io.apimachinery.pkg.apis.meta.v1.Time |  DiscoveryTimestamp is the time at which the Freight was first discovered from its source. This is distinct from metadata.creationTimestamp, which is set by Kubernetes and cannot be controlled by users. |
