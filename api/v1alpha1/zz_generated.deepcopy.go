...
func (in FreightHistory) DeepCopyInto(out *FreightHistory) {
	{
		...
		*out = make(FreightHistory, len(*in))
		for i := range *in {
			if (*in)[i] != nil {
				in, out := &(*in)[i], &(*out)[i]
				*out = make(FreightSelection, len(*in))
				for key, val := range *in {
					var outVal *FreightReference
					if val == nil {
						(*out)[key] = nil
					} else {
						inVal := (*in)[key]
...
// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in FreightSelection) DeepCopyInto(out *FreightSelection) {
	{
		in := &in
		*out = make(FreightSelection, len(*in))
		for key, val := range *in {
...
// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new FreightSelection.
func (in FreightSelection) DeepCopy() FreightSelection {
	if in == nil {
		return nil
	}
	out := new(FreightSelection)
	in.DeepCopyInto(out)
	return *out
}
