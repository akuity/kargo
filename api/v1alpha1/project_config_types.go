...
type GitHubWebhookReceiverConfig struct {
	// SecretRef contains a reference to a Secret in the same namespace as the
	// ProjectConfig.
	//
	// The Secret is expected to contain a `token` key with the secret token
	// configured for in GitHub for the webhook. For more information about this
	// token, please refer to the GitHub documentation:
