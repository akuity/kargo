...
func feature() features.Feature {
	...
	feature.Assess("yaml-update updates the parsed field",
		func(ctx context.Context, t *testing.T, _ *envconf.Config) context.Context {
			...
			if !ok {
				t.Fatalf("promotion output %q is missing originalImage; state: %v",
					"output", promotion.Status.GetState())
			}
			updated, ok := utils.PromotionStepOutput(promotion, "output", "updatedImage")
			if !ok {
				t.Fatalf("promotion output %q is missing updatedImage; state: %v",
					"output", promotion.Status.GetState())
			}
			expected, ok := utils.PromotionStepOutput(promotion, "output", "expectedImage")
			if !ok {
				t.Fatalf("promotion output %q is missing expectedImage; state: %v",
					"output", promotion.Status.GetState())
			}
