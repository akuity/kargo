...
func TestFetchBestReleases(t *testing.T) {
...
func TestPickBestReleases(t *testing.T) {
	t.Run("nil input returns empty", func(t *testing.T) {
		results := pickBestReleases(nil)
	...
	t.Run("picks latest patch per minor, sorted descending", func(t *testing.T) {
		...
		assert.False(t, results[2].Latest)
	})

	t.Run("excludes pre-1.0 releases", func(t *testing.T) {
		raw := []Release{
			{Version: v("0.9.0"), CLIBinaries: CLIBinaries{}},
			{Version: v("1.0.0"), CLIBinaries: CLIBinaries{}},
		}
