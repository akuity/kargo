...
func (s *server) QueryFreight(
	...
	if req.Msg.GetStage() != "" {
		...
		if err != nil {
			return nil, connect.NewError(connect.CodeUnknown, errors.Wrap(err, "get available freight for stage"))
		}
	} else {
		...
		if err := s.listFreightFn(
			...
			return nil, connect.NewError(connect.CodeUnknown, errors.Wrap(err, "list freights"))
		}
