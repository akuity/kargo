...
// NB(hidde): Copied from https://github.com/kubernetes-sigs/controller-runtime/blob/2070bf4ee04c23ebae5f3d1e639567d9c160a7a4/pkg/controller/controllertest/testing.go
// to unblock us from updating Kubernetes. Can be removed once upstream
// has released a version with this change.
// nolint: lll
...
func (q *TypedQueue[request]) AddAfter(item request, _ time.Duration) {
	q.Add(item)
}
...
func (q *TypedQueue[request]) Forget(_ request) {}

// NumRequeues implements RateLimitingInterface.  TODO(community): Implement this.
func (q *TypedQueue[request]) NumRequeues(_ request) int {
	return 0
}
