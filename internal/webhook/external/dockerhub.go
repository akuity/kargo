...
func (d *dockerhubWebhookReceiver) GetHandler() http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// TODO(krancour): We should be normalizing this. We do not yet have any
		// normalization logic for image URLs, and we've gotten away with it
		// until now, but it's important here because, for example:
		//   - docker.io/example/repo == example/repo
		//   - docker.io/nginx == library/nginx == nginx
		repoURL := payload.Repository.RepoName
