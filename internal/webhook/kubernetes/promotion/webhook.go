...
func (w *webhook) recordPromotionCreatedEvent(
	...
	evt := kargoEvent.NewPromotionEvent(fmt.Sprintf("Promotion created for Stage %q by %q",
		p.Spec.Stage,
		actor), actor, p, f)
	if err := w.sender.Send(ctx, evt.ToCloudEvent(kargoapi.EventTypePromotionCreated)); err != nil {
