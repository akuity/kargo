...
func (w *webhook) recordFreightApprovedEvent(
	...
	evt := event.NewFreightEvent(actor, f, stageName, fmt.Sprintf("Freight approved for Stage %q by %q",
		stageName,
		actor,
	))
	if err := w.sender.Send(ctx, evt.ToCloudEvent(kargoapi.EventTypeFreightApproved)); err != nil {
