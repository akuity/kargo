...
func Test_gitPRMerger_isRetryableError(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			err:      errors.New("API Error: The pull request cannot be merged because required status checks are still pending completion. Please wait for all checks to finish."),
			expected: true,
		},
		{
			name:     "Complex error with non-retryable pattern",
			err:      errors.New("API Error: Pull request #456 not found in repository owner/repo. Verify the pull request number and repository access."),
			expected: false,
		},
		{
		...
		// Real-world API response patterns
		{
			...
			err:      errors.New("PUT https://api.github.com/repos/owner/repo/pulls/123/merge: 422 Pull request is not mergeable due to failing required status checks"),
			expected: true,
		},
		{
		...
		},
		{
			...
			err:      errors.New("TF400898: An Internal Error Occurred. Activity Id: abc123. Required pull request policy 'Minimum number of reviewers' has not been satisfied"),
			expected: true,
		},
		{
		...
		},
		{
			...
			err:      errors.New("403 API rate limit exceeded for user ID 12345. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.)"),
			expected: false, // 403 is non-retryable due to first check
		},
		{
			name:     "Complex Jenkins/CI integration error",
			err:      errors.New("Merge blocked: Required status check 'continuous-integration/jenkins/pr-merge' is pending. The PR will be ready for merge once all required checks pass."),
			expected: true,
		},
		{
			name:     "Branch protection with specific rule",
			err:      errors.New("Cannot merge: 2 of 3 required status checks are still pending: 'ci/circleci: test', 'ci/circleci: lint'"),
			expected: true,
		},
		{
			name:     "GitLab WIP merge request",
			err:      errors.New("405 Method Not Allowed: Cannot accept this merge request, it is a Work in Progress. Remove the WIP prefix from the title to allow it to be merged"),
			expected: true,
		},
		{
			name:     "Azure build validation failure",
			err:      errors.New("The pull request cannot be completed because one or more policies are not satisfied: Build validation policy requires a successful build before completing the pull request"),
			expected: true,
		},
		...
		// Performance and edge cases
		{
			...
			err:      errors.New("This is a very long error message that contains many words and describes a complex scenario where the pull request cannot be merged due to various policy requirements and the fact that required status checks are still pending completion from the continuous integration system which is currently processing the latest changes pushed to the source branch and will need additional time to complete its validation processes"),
			expected: true,
		},
		{
			name:     "JSON formatted error response",
			err:      errors.New(`{"message":"Pull request is not mergeable","documentation_url":"https://docs.github.com/rest/reference/pulls#merge-a-pull-request"}`),
			expected: true,
		},
		{
			name:     "URL with error in query params",
			err:      errors.New("GET https://gitlab.example.com/api/v4/projects/123/merge_requests/456/merge?merge_when_pipeline_succeeds=true returned 409: Merge request cannot be merged"),
			expected: true,
		},
		{
			name:     "Non-English error message (hypothetical)",
			err:      errors.New("La demande de fusion ne peut pas être fusionnée"), // French: "The merge request cannot be merged"
			expected: false,                                                         // Non-English patterns not explicitly supported
		},
	}
