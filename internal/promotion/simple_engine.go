...
func (e *simpleEngine) executeSteps(
	...
	for i := promoCtx.StartFromStep; i < int64(len(steps)); i++ {
		...
		if e.shouldSkipStep(ctx, promoCtx, step, stepExecMeta) {
			continue
		}
		...
		// Execute the step.
		result, err := e.executeStep(ctx, promoCtx, step, runner, workDir)
		...
		if !e.determineStepCompletion(step, runner, stepExecMeta, err) {
			// The step is still running, so we need to w
			return Result{
...
func (e *simpleEngine) shouldSkipStep(
	...
	skip, err := step.Skip(ctx, e.kargoClient, promoCtx)
	if err != nil {
		meta.Status = kargoapi.PromotionStepStatusErrored
...
func (e *simpleEngine) executeStep(
	...
	stepCtx, err := e.prepareStepContext(ctx, promoCtx, step, workDir)
	if err != nil {
...
func (e *simpleEngine) prepareStepContext(
	...
	stepCfg, err := step.GetConfig(ctx, e.kargoClient, promoCtx)
	if err != nil {
